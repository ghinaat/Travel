...
@stop
@section('content')
<form action="{{route('pktwisata.store')}}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-12">
            <div class="card">
...
@error('harga_per_pack') is-invalid @enderror" id="harga_per_pack" placeholder="Harga" name="harga_per_pack"
                            value="{{ number_format(old('harga_per_pack'),2,',','.') }}">
...
btn-default">
    ...
    //         // Menghilangkan karakter selain angka dan titik desimal
    //         var value = $(this).val().replace(/[^0-9.]/g, '');

    //         // Menghapus titik desimal yang berlebihan
    //         var parts = value.toString().split('.');
    //         parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.');

    //         // Menggabungkan angka dengan titik desimal
    //         $(this).val(parts.join('.'));
    //     });
    // });
    </script>
